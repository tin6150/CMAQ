...
C $Header: /global/home/groups-sw/ac_seasonal/lbastien/CMAQ/v4.5-ADJ/models/STENEX/src/se_snl/se_gather_module.f,v 1.1.1.1 2005/09/09 18:58:42 sjr Exp $
...
C -- allocate data
        allocate (bufptr(gn*se_numprocs), stat=allocate_status)
...
C -- allocate data
        allocate (locnarray(0:se_numprocs-1), stat=allocate_status)
        if (allocate_status .ne. 0) then
           print *, ' Subroutine SE_SETUP_GATHER: ',
...
C -- PE 0 colloect ptr information from each PE
     ...
     &                   mpi_integer, 0, mpi_comm_world, error)

C -- PE 0 colloect n from each PE
        call mpi_gather (n, 1, mpi_integer, locnarray(se_my_pe), 1,
     &                   mpi_integer, 0, mpi_comm_world, error)
...
C -- gathering data by using se_slice function
        do j = 1, locn
           i = locptr(j) 
           if ((locpec(j) .ge. 0) .or. (se_my_pe .eq. 0)) then
