...
! Modified by Lucas A. J. Bastien (February 17th 2015) so that the
! KPP-generated code can be interfaced with CMAQ. In particular, this involves
! skipping the calculations of photolysis rate coefficients here since they are
! calculated in the subroutine CHEM, using CMAQ's run-time photolysis rate
! coefficients calculation capabilities (phot.F), as opposed to the
! calculations just being based on SUN as is the case in the original
! KPP-generated code

MODULE saprc99ros_Rates
...
! End INLINED RCONST
  ...
  ! First update the non-photolysis reaction rates
